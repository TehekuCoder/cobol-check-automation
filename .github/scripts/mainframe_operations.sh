...
sshpass -e ssh $SSH_OPTS "${SSH_USERNAME}@${SSH_HOST}" \
  "chmod +x ${REMOTE_DIR}/remote_cobolcheck.sh && tr -d '\r' < ${REMOTE_DIR}/remote_cobolcheck.sh > ${REMOTE_DIR}/remote_cobolcheck_fixed.sh && chmod +x ${REMOTE_DIR}/remote_cobolcheck_fixed.sh && zsh ${REMOTE_DIR}/remote_cobolcheck_fixed.sh ${LOWERCASE_USERNAME} ${SSH_USERNAME}"
